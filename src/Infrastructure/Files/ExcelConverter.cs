...
namespace Anubis.Infrastructure.Files
{
    public class ExcelConverter : IExcelConverter
    {
        ...
        public byte[] Convert(IEnumerable<dynamic> data, string sheetName = "Sheet 1", Dictionary<string, string> columns = null)
        {
            ...
            foreach (var d in data)
            {
                ...
                foreach (var prop in props)
                {
                    ...
                    if (!prop.Name.ToLower().Contains("shipmentiddock") && decimal.TryParse(value, out decimal dec))
                    {
        ...
        public byte[] ConvertCsv(IEnumerable<dynamic> data, Dictionary<string, string> columns = null)
        {
            ...
            {
                var value = prop.Name;
                if (columns?.ContainsKey(prop.Name) == true && columns?[prop.Name] != null)
                    value = columns[prop.Name];
            ...
            foreach (var d in data)
            {
                ...
                foreach (var prop in props)
                {
                    var value = System.Convert.ToString(prop.PropertyType == typeof(DateTime?) ? prop.GetValue(d) != null ? prop.GetValue(d).ToString("MM/dd/yyyy") : "N/A" : prop.GetValue(d));
